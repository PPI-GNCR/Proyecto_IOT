...
///////////////////////////////////////////////////////////////////
static void getTelemetryPayload(char* buffer, size_t buffer_size) {
    snprintf(buffer, buffer_size, "{\"Temp\": %.2f, \"Hum\": %.2f, \"Volt\": %.2f, \"Puerta\": %d}", 
        temperatura,    // Asegúrate de que sea float
        humedad,       // Asegúrate de que sea float
        voltaje,             // Float
        puertaAbierta);      // Entero
}

static void sendTelemetry() {
    char telemetry_payload[100]; // Tamaño ajustado para contener el payload JSON
    getTelemetryPayload(telemetry_payload, sizeof(telemetry_payload));

    Logger.Info("Enviando telemetría: ");
    Logger.Info(telemetry_payload);

    // Obtener el topic de publicación
    if (az_result_failed(az_iot_hub_client_telemetry_get_publish_topic(
            &client, NULL, telemetry_topic, sizeof(telemetry_topic), NULL)))
    {
        Logger.Error("Error al obtener el tópico de telemetría");
        return;
    }

    // Publicar los datos en Azure IoT
    if (esp_mqtt_client_publish(
            mqtt_client,
            telemetry_topic,
            telemetry_payload,               // Payload como char*
            strlen(telemetry_payload),        // Longitud del payload
            MQTT_QOS1,
            DO_NOT_RETAIN_MSG)
        == 0)
    {
...
void loop() {
  ...
  else if (millis() > next_telemetry_send_time_ms)
  {
    ...
    int valorPuertaVerdadero = valorPuerta;
    Serial.print("Puerta: ");
